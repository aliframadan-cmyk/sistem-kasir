...
import { 
  ...
  Trash2, X, CheckCircle2, ReceiptText, PlusCircle, Edit, LogOut, Printer, Eraser, ScanBarcode, TrendingUp, AlertTriangle, Wallet, ArrowRight
} from 'lucide-react';
...
const App = () => {
  ...
  const [showSuccess, setShowSuccess] = useState(false); // Sukses Transaksi
  const [showSaveSuccess, setShowSaveSuccess] = useState(false); // Sukses Simpan Barang (BARU)
  ...
  const stats = useMemo(() => {
    ...
    const last7Days = [...Array(7)].map((_, i) => {
        const d = new Date(); d.setDate(d.getDate() - i); return d.toLocaleDateString('id-ID'); 
    }).reverse();

    const chartData = last7Days.map(date => {
        const omsetHariIni = riwayat.filter(r => r.tanggal === date).reduce((acc, curr) => acc + curr.total, 0);
  ...
  // --- LOGIKA INVENTORY (SIMPAN BARANG) ---
  const handleSimpanProduk = () => {
    ...
    // TUTUP MODAL FORM DAN TAMPILKAN POPUP SUKSES
    setShowAddModal(false);
    setShowSaveSuccess(true);
  };
  
  const handleHapusProduk = (id: number) => { 
      if (confirm("Yakin hapus produk ini?")) { setProdukList(produkList.filter(p => p.id !== id)); }
  };
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] font-sans overflow-hidden">
      ...
      <div id="struk-print" className="hidden font-mono text-sm max-w-[80mm] mx-auto bg-white p-4">
        {lastTrx && (
          <div className="text-center">
             ...
             <div className="text-left">{lastTrx.items.map(i => (<div key={i.id} className="flex justify-between mb-1"><span>{i.nama} <br/><span className="text-[10px]">{i.qty} x {i.hargaEcer.toLocaleString()}</span></span><span>{i.subtotal.toLocaleString()}</span></div>))}</div>
             <hr className="border-dashed border-black my-2"/>
            ...
            <div className="flex gap-4">
              {activeTab === 'pos' && (
                <form onSubmit={handleScanBarcode} className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><ScanBarcode className="text-blue-500" size={20}/></div>
            ...
            <div className="space-y-6 max-w-7xl mx-auto pb-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                        <div className="flex items-center gap-3 mb-8"><div className="p-2 bg-slate-100 rounded-lg"><TrendingUp size={20} className="text-slate-600"/></div><div><h3 className="font-bold text-slate-800 text-lg">Grafik Penjualan</h3><p className="text-xs text-slate-400">7 Hari Terakhir</p></div></div>
                        <div className="flex items-end gap-3 h-64 w-full px-2">{stats.chartData.map((d, i) => { const heightPercent = d.total === 0 ? 2 : (d.total / stats.maxOmset) * 100; return (<div key={i} className="flex-1 flex flex-col items-center gap-2 group cursor-pointer"><div className="relative w-full bg-slate-50 rounded-xl flex items-end h-full overflow-hidden hover:bg-slate-100 transition-all"><div style={{ height: `${heightPercent}%` }} className={`w-full rounded-t-xl transition-all duration-700 relative ${d.total > 0 ? 'bg-blue-600 group-hover:bg-blue-500' : 'bg-slate-200'}`}><div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 shadow-lg">Rp {d.total.toLocaleString('id-ID')}</div></div></div><span className="text-xs font-bold text-slate-400">{d.date}</span></div>) })}</div>
                    </div>
                    <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                        ...
                        <div className="space-y-4">{stats.topProduk.length === 0 ? <p className="text-slate-400 text-sm text-center py-10">Belum ada data.</p> : stats.topProduk.map(([nama, qty], idx) => (<div key={idx} className="flex justify-between items-center pb-3 border-b border-dashed border-slate-100 last:border-0 hover:bg-slate-50 p-2 rounded-lg transition-colors"><div className="flex items-center gap-3"><div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${idx === 0 ? 'bg-yellow-100 text-yellow-700' : 'bg-slate-100 text-slate-500'}`}>{idx+1}</div><span className="font-bold text-slate-700 text-sm">{nama}</span></div><span className="font-black text-blue-600 text-sm">{qty} Pcs</span></div>))}</div>
                    </div>
                </div>
            </div>
          )}
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-2 scrollbar-thin">
              ...
              {keranjang.map(item => (
                  <div key={item.id} className="flex justify-between items-start bg-slate-50 p-4 rounded-2xl group border border-transparent hover:border-blue-200 transition-all">
                      <div><p className="font-bold text-sm text-slate-700 line-clamp-1">{item.nama}</p><div className="flex items-center gap-2 mt-1"><span className="text-xs font-bold bg-white px-2 py-1 rounded border">{item.qty}x</span><span className="text-xs text-slate-400">@ {item.hargaEcer.toLocaleString()}</span></div></div>
                      <div className="text-right"><p className="font-bold text-slate-700 text-sm">Rp {item.subtotal.toLocaleString()}</p><button onClick={() => setKeranjang(keranjang.filter(i => i.id !== item.id))} className="text-red-400 hover:text-red-600 text-[10px] mt-1 font-bold">HAPUS</button></div>
                  </div>
              ))}
            </div>
      ...
      {/* 3. MODAL SUKSES SIMPAN DATA STOK (BARU) */}
      {showSaveSuccess && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md flex items-center justify-center z-[110] p-4">
            <div className="bg-white p-8 rounded-[2rem] text-center shadow-2xl w-full max-w-xs animate-bounce-in">
                <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <CheckCircle2 size={32} className="text-blue-600"/>
                </div>
                <h2 className="text-xl font-black text-slate-800 mb-2">Sukses Disimpan!</h2>
                <p className="text-slate-400 text-xs mb-6">Data produk dan stok berhasil diperbarui ke database.</p>
                <button 
                ...
                >
                    MANTAP <ArrowRight size={18}/>
                </button>
            </div>
        </div>
      )}
      
      {/* 4. MODAL WARNING EMPTY */}
