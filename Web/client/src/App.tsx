...
import { 
  ...
  Trash2, X, CheckCircle2, ReceiptText, AlertCircle, PlusCircle, Edit, LogOut, Lock, Printer, Eraser
} from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      ...
      if (savedHistory) {
        try { setRiwayat(JSON.parse(savedHistory)); } catch (e) { console.error(e); }
      }
    }
  }, [isLoggedIn]);
  ...
  const hapusSatuRiwayat = (id: string) => {
    if (confirm("Hapus transaksi ini dari riwayat?")) {
      const newRiwayat = riwayat.filter(h => h.id !== id);
      setRiwayat(newRiwayat);
      localStorage.setItem('riwayat_transaksi', JSON.stringify(newRiwayat));
    }
  };

  const hapusSemuaRiwayat = () => {
    if (confirm("APAKAH ANDA YAKIN? Semua data riwayat akan dihapus permanen!")) {
      setRiwayat([]);
  ...
  const handleBayar = async () => {
    ...
    try {
      ...
      const payload = { 
        items: keranjang.map(item => ({ id: item.id, qty: item.qty, name: item.nama, price: item.hargaEcer, unit: 'Pcs' })),
        total: totalBelanja, customerType: 'General'
      };
    ...
    <div className="flex h-screen bg-[#F8FAFC] font-sans overflow-hidden">
      <style>{`
        @media print {
          ...
          #struk-print { display: block !important; position: absolute; left: 0; top: 0; width: 100%; margin: 0; padding: 10px; background: white; color: black; }
      ...
      <div id="struk-print" className="hidden font-mono text-sm max-w-[80mm] mx-auto bg-white">
        {lastTrx && (
          <div className="text-center pb-8 pt-4">
            ...
            <div className="flex justify-between text-xs mb-2"><span>{lastTrx.id}</span><span>{lastTrx.tanggal.split(' ')[0]}</span></div>
            <hr className="border-dashed border-black mb-2"/>
            <div className="text-left space-y-2 mb-4">
              {lastTrx.items.map((item, idx) => (
                <div key={idx}>
                  <div className="font-bold">{item.nama}</div>
                  <div className="flex justify-between"><span>{item.qty} x {item.hargaEcer.toLocaleString()}</span><span>{item.subtotal.toLocaleString()}</span></div>
                </div>
              ))}
            </div>
            <hr className="border-dashed border-black mb-2"/>
            <div className="flex justify-between font-black text-lg mb-8"><span>TOTAL</span><span>Rp {lastTrx.total.toLocaleString()}</span></div>
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex items-center gap-4">
              {activeTab === 'history' && riwayat.length > 0 && (
                <button onClick={hapusSemuaRiwayat} className="flex items-center gap-2 px-4 py-2.5 bg-red-50 text-red-600 rounded-xl font-bold text-sm hover:bg-red-100 transition-colors">
                  <Eraser size={18}/> Hapus Semua
                </button>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {produkList.filter(p => p.nama.toLowerCase().includes(search.toLowerCase())).map(p => (
                <div key={p.id} onClick={() => {
                  ...
                  if(p.stok <= 0 || (exist && exist.qty >= p.stok)) return alert("Stok Habis");
          ...
          {activeTab === 'history' && (
            <div className="space-y-4 max-w-4xl">
              {riwayat.length === 0 ? (
                <div className="text-center py-20 bg-white rounded-[2rem] border-dashed border-2 border-slate-200"><p className="text-slate-400 font-bold">Belum ada riwayat transaksi</p></div>
              ) : (
                riwayat.filter(h => h.id.toLowerCase().includes(search.toLowerCase())).map(h => (
                  <div key={h.id} className="bg-white p-5 rounded-2xl border border-slate-100 flex justify-between items-center shadow-sm group">
                    <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <button onClick={() => hapusSatuRiwayat(h.id)} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all opacity-0 group-hover:opacity-100">
                        <Trash2 size={20}/>
                      </button>
                    </div>
                  </div>
      ...
      {/* MODALS (Success, Empty, Add Product) */}
      {showSuccess && (
        <div className="fixed inset-0 bg-blue-900/40 backdrop-blur-md flex items-center justify-center z-[100]">
          <div className="bg-white p-10 rounded-[3rem] text-center shadow-2xl w-full max-w-sm">
            ...
            <div className="space-y-3 mt-6">
              <button onClick={() => window.print()} className="w-full bg-slate-800 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2"><Printer size={20}/> CETAK STRUK</button>
              <button onClick={() => setShowSuccess(false)} className="w-full bg-slate-100 text-slate-600 py-4 rounded-xl font-bold">Transaksi Baru</button>
            </div>
          </div>
        </div>
      )}
