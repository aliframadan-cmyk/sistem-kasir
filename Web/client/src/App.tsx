...
import { 
  ...
  Trash2, X, CheckCircle2, ReceiptText, AlertCircle, PlusCircle, Edit, LogOut, Lock, Printer, Eraser, ScanBarcode
} from 'lucide-react';
...
const App = () => {
  ...
  const [barcodeInput, setBarcodeInput] = useState(""); // Input khusus Scanner
  ...
  // Ref untuk auto-focus input barcode
  ...
  const fetchProducts = async () => {
    try {
      ...
      const formattedData = data.map((item: any) => ({
        ...
        barcode: item.barcode || "", // Mapping Barcode
        originalData: item
      }));
  ...
  // Agar input barcode selalu fokus saat masuk tab POS
  useEffect(() => {
  ...
  const handleScanBarcode = (e: React.FormEvent) => {
    e.preventDefault(); // Mencegah reload halaman saat Scanner tekan Enter
  ...
  // --- LOGIKA INVENTORY (SIMPAN PRODUK) ---
  const handleSimpanProduk = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        stockPcs: newItem.stockPcs,
        pricePcs: newItem.pricePcs,
        barcode: newItem.barcode // Kirim Barcode ke Backend
      };
  ...
  const hapusSemuaRiwayat = () => {
    if(confirm("Hapus SEMUA riwayat?")) { setRiwayat([]); localStorage.removeItem('riwayat_transaksi'); }
  };
  ...
  const handleBayar = async () => {
    ...
    try {
      ...
      const payload = { 
        items: keranjang.map(item => ({ id: item.id, qty: item.qty, name: item.nama, price: item.hargaEcer })),
        total: totalBelanja 
      };
  ...
  // LOGIN PAGE
  if (!isLoggedIn) return <div className="flex h-screen items-center justify-center bg-slate-900 text-white cursor-pointer" onClick={() => {localStorage.setItem('kasir_user','true'); setIsLoggedIn(true);}}>Klik untuk Login (Simulasi)</div>;
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] font-sans overflow-hidden">
      ...
      <div id="struk-print" className="hidden font-mono text-sm max-w-[80mm] mx-auto bg-white p-4">
        {lastTrx && (
          <div className="text-center">
             <h2 className="font-bold">TOKO SEMBAKO</h2>
      ...
      <aside className="w-20 bg-[#0F172A] flex flex-col items-center py-6 gap-4 z-20 no-print">
        ...
        <button onClick={() => setActiveTab('pos')} className={`p-3 rounded-xl ${activeTab === 'pos' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}><ShoppingCart size={24}/></button>
        <button onClick={() => setActiveTab('inventory')} className={`p-3 rounded-xl ${activeTab === 'inventory' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}><LayoutGrid size={24}/></button>
        <button onClick={() => setActiveTab('history')} className={`p-3 rounded-xl ${activeTab === 'history' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}><History size={24}/></button>
      </aside>
      ...
      <div className="flex-1 flex overflow-hidden">
        <main className="flex-1 overflow-y-auto p-8">
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-3xl font-black text-slate-800 uppercase">{activeTab === 'pos' ? 'Kasir' : activeTab === 'inventory' ? 'Stok' : 'Riwayat'}</h1>
            
            <div className="flex gap-4">
              {/* --- INPUT SCANNER BARCODE (Hanya muncul di POS) --- */}
              {activeTab === 'pos' && (
                <form onSubmit={handleScanBarcode} className="relative w-64 group">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <ScanBarcode className="text-blue-500 group-focus-within:text-blue-600" size={20}/>
                    </div>
                    <input 
                        ref={barcodeInputRef}
                        type="text" 
                        className="w-full pl-10 pr-4 py-2.5 bg-blue-50 border-2 border-blue-200 focus:border-blue-600 text-slate-800 rounded-xl outline-none font-bold placeholder-blue-300 transition-all"
              ...
              <div className="relative w-64">
                ...
                <input type="text" placeholder="Cari Nama..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl outline-none focus:border-blue-500" onChange={(e) => setSearch(e.target.value)}/>
              </div>
            </div>
          </div>
          ...
          {activeTab === 'inventory' && (
            <div>
              <div className="flex justify-end mb-4">
                <button onClick={() => { setEditId(null); setNewItem({name:"",category:"Sembako",stockPcs:"",pricePcs:"",barcode:""}); setShowAddModal(true); }} className="flex items-center gap-2 bg-blue-600 text-white px-5 py-3 rounded-xl font-bold shadow-lg"><PlusCircle size={20}/> Tambah Produk</button>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {produkList.map(p => (
                      <tr key={p.id} className="hover:bg-slate-50">
                        <td className="px-6 py-5 font-mono text-xs text-slate-500">{p.barcode || "-"}</td>
                        <td className="px-6 py-5 font-bold text-slate-700">{p.nama}</td>
                        <td className="px-6 py-5 text-center font-bold text-blue-600">{p.stok}</td>
                        <td className="px-6 py-5 text-right font-black">Rp {p.hargaEcer.toLocaleString('id-ID')}</td>
                        <td className="px-6 py-5 text-center flex justify-center gap-2">
                          <button onClick={() => { setEditId(p.id); setNewItem({name:p.originalData.name,category:p.originalData.category,stockPcs:p.originalData.stockPcs,pricePcs:p.originalData.pricePcs,barcode:p.barcode}); setShowAddModal(true); }} className="p-2 bg-blue-50 text-blue-600 rounded-lg"><Edit size={18}/></button>
          ...
          {activeTab === 'history' && (
            <div className="space-y-4 max-w-4xl">
              {riwayat.length > 0 && <button onClick={hapusSemuaRiwayat} className="mb-4 text-red-500 text-sm font-bold flex items-center gap-2 hover:bg-red-50 px-4 py-2 rounded-lg w-fit"><Eraser size={16}/> Hapus Semua Riwayat</button>}
              {riwayat.map(h => (
                <div key={h.id} className="bg-white p-5 rounded-2xl border border-slate-100 flex justify-between items-center shadow-sm">
                  ...
                  <div className="text-right">
                    ...
                    <button onClick={() => { setLastTrx(h); setTimeout(() => window.print(), 100); }} className="text-xs font-bold text-slate-400 hover:text-blue-600 flex items-center gap-1 justify-end mt-1"><Printer size={12}/> CETAK</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'pos' && (
          <aside className="w-96 bg-white border-l border-slate-100 p-8 flex flex-col shadow-2xl">
            ...
            <div className="pt-6 border-t border-dashed border-slate-200 mt-6 space-y-4">
              ...
              <button onClick={handleBayar} disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-5 rounded-2xl font-black shadow-xl">BAYAR SEKARANG</button>
            </div>
          </aside>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-blue-900/40 backdrop-blur-md flex items-center justify-center z-[100]">
          <div className="bg-white p-10 rounded-[3rem] text-center shadow-2xl w-full max-w-sm">
            ...
            <div className="space-y-3 mt-6">
              <button onClick={() => window.print()} className="w-full bg-slate-800 text-white py-4 rounded-xl font-bold flex items-center gap-2 justify-center"><Printer size={20}/> CETAK STRUK</button>
              <button onClick={() => setShowSuccess(false)} className="w-full bg-slate-100 text-slate-600 py-4 rounded-xl font-bold">Tutup</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white w-full max-w-lg rounded-[2rem] p-8 shadow-2xl">
            ...
            <div className="space-y-4">
                {/* INPUT BARCODE DI MODAL */}
                <div className="relative">
                    <ScanBarcode className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18}/>
                    <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-3 font-bold font-mono" value={newItem.barcode} onChange={e=>setNewItem({...newItem,barcode:e.target.value})} placeholder="Scan Barcode Disini..."/>
                </div>
                
                <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold" value={newItem.name} onChange={e=>setNewItem({...newItem,name:e.target.value})} placeholder="Nama Produk"/>
                <div className="grid grid-cols-2 gap-4">
                    <input type="number" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold" value={newItem.stockPcs} onChange={e=>setNewItem({...newItem,stockPcs:e.target.value})} placeholder="Stok"/>
                    <input type="number" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold" value={newItem.pricePcs} onChange={e=>setNewItem({...newItem,pricePcs:e.target.value})} placeholder="Harga"/>
                </div>
            </div>
            <div className="grid grid-cols-2 gap-4 mt-8">
                <button onClick={()=>setShowAddModal(false)} className="py-4 font-bold text-slate-400">Batal</button>
                <button onClick={handleSimpanProduk} className="bg-blue-600 text-white py-4 rounded-xl font-black shadow-lg">SIMPAN</button>
            </div>
          </div>
        </div>
      )}
